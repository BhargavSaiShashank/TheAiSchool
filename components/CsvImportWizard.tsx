...
export default function CsvImportWizard({
  ...
  const [step, setStep] = useState<"upload" | "map" | "processing" | "success">(
    "upload",
  );
    ...
    Papa.parse(loaded, {
      ...
      complete: (results) => {
        ...
        heads.forEach((h) => {
          ...
          if (lh.includes("title") || lh.includes("job"))
            initial["jobTitle"] = h;
        });
        setMappings(initial);
        setStep("map");
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-zinc-500 hover:text-white p-1 rounded-full bg-white/5 z-10 transition"
        >
          <X size={16} />
        </button>
        ...
        <div className="p-6 border-b border-white/5 bg-zinc-950/50">
          ...
          <p className="text-xs text-zinc-400 mt-1 font-medium">
            Upload raw CSV documents, map logic trees, and inject telemetry
            safely.
          </p>
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === "upload" && (
              ...
              >
                ...
                <h3 className="text-white font-semibold mb-1">
                  Strike to browse or drag & drop
                </h3>
                <p className="text-xs text-zinc-500 font-mono">
                  Limit constraint: 25MB • RAW CSV ONLY
                </p>
              </motion.div>
            )}
            ...
            {step === "map" && (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-emerald-100 font-semibold">
                        Payload locked: "{file?.name}"
                      </p>
                      <p className="text-[10px] font-mono text-emerald-400/70 uppercase">
                        {headers.length} static attributes identified
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <h3 className="text-xs font-bold font-mono text-zinc-400 uppercase tracking-widest mb-4 border-b border-zinc-800 pb-2">
                  Logic Association Matrix
                </h3>
                ...
                <div className="space-y-3 mb-8">
                  {TARGET_FIELDS.map((field) => (
                    <div
                      key={field.key}
                      className="flex items-center gap-4 p-3.5 rounded-xl bg-zinc-900/40 border border-white/5"
                    >
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-zinc-900">
                  <button
                    onClick={onClose}
                    className="px-5 py-2.5 text-sm text-zinc-400 font-medium hover:text-white"
                  >
                    Cancel
                  </button>
                  <button
            ...
            {step === "processing" && (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-white mb-2 tracking-tight">
                  Transmitting Data Node Packets...
                </h3>
                <p className="text-sm text-zinc-500 max-w-xs mx-auto">
                  Streaming validated payloads directly into destination cluster
                  arrays. Do not sever the connection.
                </p>
              </motion.div>
            )}
            ...
            {step === "success" && (
              ...
              >
                ...
                <h3 className="text-2xl font-extrabold text-white mb-2">
                  Grid Integration Absolute!
                </h3>
                <p className="text-sm text-zinc-400 mb-8">
                  Stream processing has legally concluded with the following
                  yield metrics:
                </p>
                ...
                <div className="grid grid-cols-3 gap-4 max-w-md mx-auto mb-8 text-center">
                  ...
                  <div className="p-4 bg-red-900/10 border border-red-900/30 rounded-xl">
                    <span className="block text-xs font-mono font-bold text-red-400 uppercase">
                      Violations
                    </span>
