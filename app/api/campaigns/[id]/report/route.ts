...
export async function GET(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [
      ...
      prisma.campaignSend.count({ where: { campaign_id: campaignId, status: "delivered" } }),
      ...
      prisma.campaignSend.count({ where: { campaign_id: campaignId, status: "bounced" } }),
      prisma.emailEvent.count({ where: { campaign_id: campaignId, event_type: "complained" } }),
    ...
    allEvents.filter(e => e.event_type === "clicked" && e.metadata).forEach(ev => {
      try {
    ...
    const linksClicked = Object.entries(linkMap).map(([url, clicks]) => ({ url, clicks }));
    ...
    return NextResponse.json({
      ...
      metrics: {
        ...
        clickRate: totalSends > 0 ? ((clicks / totalSends) * 100).toFixed(1) : 0
      },
      charts: {
