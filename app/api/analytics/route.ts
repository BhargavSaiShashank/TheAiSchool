...
export async function GET(req: Request) {
  try {
    ...
    const uniqueOpens = await prisma.emailEvent.count({
      where: {
    ...
    const totalClicks = await prisma.emailEvent.count({
      where: {
        campaign_id: campaignId,
        event_type: "clicked",
      },
    ...
    // 2. Open Over Time (last 24 hours of campaign creation grouped in 4-hour blocks)
    const openOverTime = [];
    ...
    // 3. User Agent Distribution (Parse real user agents from database)
